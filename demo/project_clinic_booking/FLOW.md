# QuickClinic end-to-end demo flow
...
3. Create `demo/project_clinic_booking/.env` from `.env.example`.
4. Run `npm run demo:clinic:validate`.

Checkpoint: every command is healthy and the seeded users can sign in.

## 2. Client intake

1. Run `npm run demo:clinic:intake`.
2. Sign in as the customer and review the completed QuickClinic brief.
...
1. Run `npm run demo:clinic:planning` to generate payloads without submitting.
2. Review `output/architecture-submission.json` and
   `output/ui_ux-submission.json` against the live adaptive requirements.
3. Run `npm run demo:clinic:planning -- --submit` to submit both handoffs.
...
1. Run `npm run demo:clinic:implementation` to generate a payload for every task
...
3. Submit through the UI for the main demo, or run
   `npm run demo:clinic:implementation -- --submit` for the text-evidence API
   path.
